...
from .sequence import Sequence, SequenceImage


class DatasetProvider:
    def __init__(self, dataset_path: Path, mode: str = 'train', event_representation: str = 'voxel_grid',
                 nr_events_data: int = 5, delta_t_per_data: int = 20, nr_events_window=-1, nr_bins_per_data=5,
                 require_paired_data=False, normalize_event=False, separate_pol=False, semseg_num_classes=11,
                 augmentation=False, fixed_duration=False, random_crop=False):
        train_path = dataset_path / 'train_events'
        val_path = dataset_path / 'test_events'
...
class DatasetImageProvider:
    def __init__(self, dataset_path: Path, mode: str = 'train',  semseg_num_classes=11, augmentation=False,
                 random_crop=False):
        train_path = dataset_path / 'train_images'
        val_path = dataset_path / 'test_images'
        assert dataset_path.is_dir(), str(dataset_path)
        # assert train_path.is_dir(), str(train_path)
        assert val_path.is_dir(), str(val_path)
        ...
        if mode == 'train':
            ...
            for child in train_path.iterdir():
                if any(k in str(child) for k in train_sequences_namelist):
                    train_sequences.append(SequenceImage(child, 'train', semseg_num_classes, augmentation,
                                                         random_crop=random_crop))
                else:
                    continue
            ...
            for child in val_path.iterdir():
                if any(k in str(child) for k in val_sequences_namelist):
                    val_sequences.append(SequenceImage(child, 'val', semseg_num_classes, augmentation,
                                                       random_crop=random_crop))
                    # print(val_sequences)
                else:
                    continue
