...
class Trainer():
    ...
    pretrained_path : str = "./checkpoint/dff_cityscapes_resnet101.pth"):
        ...
        #     #             print("Ignoring size mismatch for parameter:", name)
        #     #         else:
    ...
    def train(self,epoch,cpt = "./checkpoint"):
        ...
        for j,(idxs,imgs,tars) in enumerate(self.trainloader):
            ...
            fuse_np = fuse[0].sigmoid_().cpu().detach().numpy()
            gt_np = tars[0].cpu().detach().numpy()
            fuse_np = np.where(fuse_np > 0.45, 1.0, 0.0)


            if j % 1000 == 0:
                fuse_path = os.path.join("./vis_fuse", f"{epoch}-{j}.png")
                gt_path = os.path.join("./vis_gt", f"{epoch}-{j}.png")
                # imgnp = imgs[0].cpu().detach().numpy()
                # print(imgnp.shape, imgnp)
        ...
        if epoch % save_epoch == 0:
            path = os.path.join(cpt,"%s_fine_%d_11.pth"%(self.datasetname,epoch))
